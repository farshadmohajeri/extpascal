{
ExtToPascal is a parser that scans ExtJS documentation, in HTML format, and that creates a Object Pascal wrapper upon ExtJS classes and widgets
    ...
    with TClass(Classes.Objects[I]) do
      if Parent <> '' then begin
        ...
        if J <> -1 then begin
          ...
          if IsExtJS3 and (Self.Name = 'Ext') and (UName = 'ExtData') then continue; // remove circular reference
...
begin
  for I := 0 to Units.Count-1 do
    with TUnit(Units.Objects[I]) do begin
      ...
      writeln(Pas, '{$IFDEF FPC}{$MACRO ON}{$ENDIF}', ^M^J);
...
begin
  ...
  if FindFirst(P + '/*.html', faAnyFile, F) = 0 then begin
    ...
    writeln('(c) 2009 by Wanderlan Santos dos Anjos, BSD license');
