    ...
    with TClass(Classes.Objects[I]) do
      if Parent <> '' then begin
        ...
        if J <> -1 then begin
          ...
          if IsExt3 and (Self.Name = 'Ext') and (UName = 'ExtData') then continue; // remove circular reference
    ...
    if (Typ <> '') and (pos('.' + Typ + '.', Types) = 0) then begin
      ...
      for I := 0 to Units.Count-1 do
        if AllClasses.IndexOf('T' + Units[I] + T) <> -1 then begin
          ...
          if IsExt3 and (Name = 'Ext')       and (Units[I] = 'ExtData') then exit; // remove circular reference
  ...
  for I := 0 to Units.Count-1 do
    with TUnit(Units.Objects[I]) do begin
      ...
      if Name = 'Ext' then begin
        writeln(Pas, 'const');
        if IsExt3 then begin
          writeln(Pas, Tab, 'IsExt3     = true;');
          writeln(Pas, Tab, 'SourcePath = ''/src'';'^M^J);
        end
        else begin
          writeln(Pas, Tab, 'IsExt3     = false;');
